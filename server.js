...
const UserSchema = new mongoose.Schema({
  ...
  // Tägliche Soll-Stunden: {0:So,1:Mo,...,6:Sa}
...
const BuchungSchema = new mongoose.Schema({
  ...
  user_id:    String, typ:String, ts:String,
...
const AntragSchema = new mongoose.Schema({
  ...
  user_id:  String, typ:{type:String,default:'urlaub'}, status:{type:String,default:'offen'},
  von:String, bis:String, tage:Number,
  datum:String, kommen:String, gehen:String, pauseMin:{type:Number,default:0},
  kommentar:{type:String,default:''}, ts:String,
});

const UebertragSchema = new mongoose.Schema({
  key:    {type:String,required:true,unique:true},
  stunden:{type:Number,default:0}, urlaub:{type:Number,default:0},
});
...
const AbwesenheitTypSchema = new mongoose.Schema({
  atid:        {type:String,required:true,unique:true},
  name:        String,
  emoji:       {type:String,default:'📅'},
  farbe:       {type:String,default:'#6b7280'},
  farbeBg:     {type:String,default:'#f3f4f6'},
  zaehltAls:   {type:String,default:'keine_arbeitszeit'}, // 'arbeitszeit' | 'keine_arbeitszeit'
  festTyp:     {type:Boolean,default:false}, // vordefiniert, nicht löschbar
});
...
const AbwesenheitTyp  = mongoose.model('AbwesenheitTyp',  AbwesenheitTypSchema);
const Settings        = mongoose.model('Settings',        SettingsSchema);
const Counter         = mongoose.model('Counter',         CounterSchema);

// ── Standard-Abwesenheitstypen anlegen falls nicht vorhanden ──────────────────
const STANDARD_TYPEN = [
  {atid:'krank',       name:'Krankheit',     emoji:'🏥', farbe:'#dc2626', farbeBg:'#fef2f2', zaehltAls:'keine_arbeitszeit', festTyp:true},
  {atid:'urlaub',      name:'Urlaub',        emoji:'🏖️', farbe:'#7c3aed', farbeBg:'#f5f3ff', zaehltAls:'keine_arbeitszeit', festTyp:true},
  {atid:'sonderurlaub',name:'Sonderurlaub',  emoji:'🎁', farbe:'#0891b2', farbeBg:'#ecfeff', zaehltAls:'keine_arbeitszeit', festTyp:false},
  {atid:'elternzeit',  name:'Elternzeit',    emoji:'👶', farbe:'#d97706', farbeBg:'#fffbeb', zaehltAls:'keine_arbeitszeit', festTyp:false},
  {atid:'geschaeftsreise',name:'Geschäftsreise',emoji:'✈️',farbe:'#2563eb',farbeBg:'#eff4ff',zaehltAls:'arbeitszeit',     festTyp:false},
  {atid:'homeoffice',  name:'Homeoffice',    emoji:'🏠', farbe:'#16a34a', farbeBg:'#f0fdf4', zaehltAls:'arbeitszeit',      festTyp:false},
];

async function initAbwesenheitTypen() {
  for (const t of STANDARD_TYPEN) {
    await AbwesenheitTyp.findOneAndUpdate({atid:t.atid}, t, {upsert:true, new:true});
  }
}
...
function getFeiertage(year, bundesland='NW') {
  ...
  if (['BB','MV','SN','ST','TH'].includes(bl)) feste[`${year}-10-31`]='Reformationstag';
  if (['BW','BY','NW','RP','SL'].includes(bl)) feste[`${year}-11-01`]='Allerheiligen';
  if (bl==='SN') feste[fmt(add(osterSonntag(year),-11))]='Buß- und Bettag';
  if (bl==='HH'||bl==='HB'||bl==='NI'||bl==='SH'||bl==='MV') feste[`${year}-10-31`]='Reformationstag';

  return feste; // { "2025-01-01": "Neujahr", ... }
}

// ── Soll-Stunden für einen Monat berechnen ────────────────────────────────────
...
async function tagesauswertung(userId, monat) {
  ...
  // Map von atid → Typ-Objekt für schnellen Zugriff
  const typMap = {};
  abwTypen.forEach(t=>{ typMap[t.atid]=t; });
  ...
  for (let day=1;day<=daysInMonth;day++) {
    ...
    // Abwesenheitsbuchung prüfen (alle bekannten Abwesenheitstypen)
    const abwEntry = entries.find(b=> typMap[b.typ] && b.typ!=='kommen' && b.typ!=='gehen' && b.typ!=='pause_start' && b.typ!=='pause_ende');
    const abwTyp   = abwEntry ? typMap[abwEntry.typ] : null;

    if (istFeiertag && !abwEntry) {
      result.push({date:dateStr,typ:'feiertag',abwTypId:null,ein:'',aus:'',pauseMin:0,netto:null,
        kommentar:feiertage[dateStr],nachbuchung:false,fehler:false,sollH,dow}); continue;
    }

    if (abwEntry && abwTyp) {
      // Bei Abwesenheit die als Arbeitszeit zählt → netto = sollH
      const netto = abwTyp.zaehltAls==='arbeitszeit' ? sollH : null;
      result.push({date:dateStr,typ:abwEntry.typ,abwTypId:abwEntry.typ,
        abwName:abwTyp.name,abwEmoji:abwTyp.emoji,abwFarbe:abwTyp.farbe,abwFarbeBg:abwTyp.farbeBg,
        zaehltAls:abwTyp.zaehltAls,
        ein:'',aus:'',pauseMin:0,netto,
        kommentar:abwEntry.kommentar,nachbuchung:false,fehler:false,sollH,dow});
      continue;
    }

    if (!istArbeitstag) {
      result.push({date:dateStr,typ:'frei',abwTypId:null,ein:'',aus:'',pauseMin:0,netto:null,
        kommentar:'',nachbuchung:false,fehler:false,sollH:0,dow}); continue;
    }
    ...
    if (!kommen.length && dateStr<=todayStr) {
      result.push({date:dateStr,typ:'fehler',abwTypId:null,ein:'',aus:'',pauseMin:0,netto:null,
        kommentar:'Keine Buchung',nachbuchung:false,fehler:true,sollH,dow}); continue;
    }
    if (!kommen.length) {
      result.push({date:dateStr,typ:'offen',abwTypId:null,ein:'',aus:'',pauseMin:0,netto:null,
        kommentar:'',nachbuchung:false,fehler:false,sollH,dow}); continue;
    }

    const ein = kommen[0].ts.slice(11,16);
    if (!gehen.length && dateStr<todayStr) {
      result.push({date:dateStr,typ:'fehler',abwTypId:null,ein,aus:'',pauseMin:0,netto:null,
        kommentar:'Kommen ohne Gehen',nachbuchung:nb,fehler:true,sollH,dow}); continue;
    }
    ...
    result.push({date:dateStr,typ:gehen.length?'normal':'aktiv',abwTypId:null,ein,aus,pauseMin,netto,
      kommentar:komm?komm.kommentar:'',nachbuchung:nb,fehler:false,sollH,dow});
  }
  return result;
}
...
app.post('/api/users', async (req,res) => {
  ...
  await User.create({id,username,name,role:role||'ma',firma:firma||'',
...
// ── FIRMEN ────────────────────────────────────────────────────────────────────
app.get('/api/firmen', async (req,res) => res.json(await Firma.find().lean()));
...
app.patch('/api/firmen/:fid', async (req,res) => {
...
app.delete('/api/firmen/:fid', async (req,res) => {
  await Firma.deleteOne({fid:req.params.fid}); res.json({ok:true});
});

// ── FEIERTAGE ─────────────────────────────────────────────────────────────────
app.get('/api/feiertage/:year', async (req,res) => {
...
// Abwesenheit mit spezifischem Datum (für Admin-Eintragung)
app.post('/api/buchen-datum', async (req,res) => {
  const {userId,typ,datum,kommentar}=req.body;
  if (!userId||!typ||!datum) return res.status(400).json({error:'Fehlt.'});
  const bid=await newId();
  await Buchung.create({bid,user_id:userId,typ,ts:`${datum}T00:00:00.000Z`,kommentar:kommentar||'',nachbuchung:true});
  broadcast('buchung',{userId,typ}); res.json({ok:true});
});
...
app.patch('/api/antraege/:id', async (req,res) => {
  ...
  if (status==='genehmigt'&&a.typ==='urlaub'&&a.von&&a.bis) {
    ...
    const arbTage=user?.arbeitstage||[1,2,3,4,5];
    let cur=new Date(a.von); const end=new Date(a.bis);
    while (cur<=end) {
...
// ── ABWESENHEITSTYPEN ─────────────────────────────────────────────────────────
app.get('/api/abwesenheittypen', async (req,res) => {
  const typen = await AbwesenheitTyp.find().lean();
  res.json(typen);
});

app.post('/api/abwesenheittypen', async (req,res) => {
  const {atid,name,emoji,farbe,farbeBg,zaehltAls} = req.body;
  if (!atid||!name) return res.status(400).json({error:'atid und name erforderlich.'});
  if (await AbwesenheitTyp.findOne({atid})) return res.status(409).json({error:'ID bereits vorhanden.'});
  await AbwesenheitTyp.create({atid,name,emoji:emoji||'📅',farbe:farbe||'#6b7280',farbeBg:farbeBg||'#f3f4f6',zaehltAls:zaehltAls||'keine_arbeitszeit',festTyp:false});
  res.json({ok:true});
});

app.patch('/api/abwesenheittypen/:atid', async (req,res) => {
  const t = await AbwesenheitTyp.findOne({atid:req.params.atid});
  if (!t) return res.status(404).json({error:'Nicht gefunden.'});
  if (req.body.name)       t.name      = req.body.name;
  if (req.body.emoji)      t.emoji     = req.body.emoji;
  if (req.body.farbe)      t.farbe     = req.body.farbe;
  if (req.body.farbeBg)    t.farbeBg   = req.body.farbeBg;
  if (req.body.zaehltAls)  t.zaehltAls = req.body.zaehltAls;
  await t.save(); res.json({ok:true});
});

app.delete('/api/abwesenheittypen/:atid', async (req,res) => {
  const t = await AbwesenheitTyp.findOne({atid:req.params.atid});
  if (!t) return res.status(404).json({error:'Nicht gefunden.'});
  if (t.festTyp) return res.status(403).json({error:'Vordefinierte Typen können nicht gelöscht werden.'});
  await AbwesenheitTyp.deleteOne({atid:req.params.atid});
  res.json({ok:true});
});
